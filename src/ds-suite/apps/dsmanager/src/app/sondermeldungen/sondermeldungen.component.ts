...
import { NoteService, DisplayService } from '@ds-suite/core'
...
export class SondermeldungenComponent implements OnInit {
  ...
  public minimizeSidebar: boolean = false
  public minimizeZuordnung: boolean = false
  ...
  saveAssignmentClick () {
    ...
    if (this.currentDisplaysChecked.filter(d => d).length === 0) {
      return
    }
  ...
  saveCurrentAssignment () {
    ...
    for (i = 0; i < this.currentDisplaysChecked.length; i++) {
      if (this.currentDisplaysChecked[i]) {
  ...
  openPreview (displayName: string, timestamp: Date) {
    ...
    if (window.location.origin.includes('localhost')) { openLink = `http://localhost:4201/#/${displayName}?timestamp=${timestamp.toISOString()}` } else { openLink = `${window.location.origin}/displays/#/${displayName}?timestamp=${timestamp.toISOString()}` }
