...
export class DisplayTemplateComponent implements OnInit, OnDestroy {
  ...
  constructor (private readonly route: ActivatedRoute, private readonly terminService: TerminService) { }
  ...
  loadTermine () {
    this.route.params.subscribe(params => {
      ...
      this.terminService.getTermine(this.display.name).subscribe(result => {
        ...
        if ((this.activeTermineIndex) > this.activeTermineCount) { this.activeTermineIndex = 1 }

        this.aktiverTermin = this.findFirstActiveTermin(tmpTermine)
        if (this.SwitchMultipleActiveTermine) { this.aktiverTermin = this.activeTermine[this.activeTermineIndex - 1] }
        ...
        if (this.scrollMode) { this.termine = this.termine.concat(tmpTermine) } else { this.termine = tmpTermine }
        this.termineLoaded()
      })
    })
  }
  ...
  ngOnInit () {
    ...
    this.updateSub = this.updateTimer.subscribe((t: any) => {
      ...
      if (this.termine.length === 0 ||
        ...
        if (this.scrollMode) { this.termine.shift() }
      }
    })
  }
  ...
  removeFinishedTermine (termine: Termin[], maxTermine: number, minFinished: number): Termin[] {
    ...
    if (termineTmp.length > maxTermine) {
      const termineFinished = termineTmp.filter(t => t.status == TerminStatus.abgeschlossen || t.status == TerminStatus.aufgehoben)
      const termineUnfinished = termineTmp.filter(t => t.status != TerminStatus.abgeschlossen && t.status != TerminStatus.aufgehoben)
