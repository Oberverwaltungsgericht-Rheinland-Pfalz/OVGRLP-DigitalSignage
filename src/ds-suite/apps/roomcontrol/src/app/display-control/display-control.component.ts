...
import { DisplayService, Resizer } from '@ds-suite/core'
...
export class DisplayControlComponent implements OnInit, AfterViewInit {
  ...
  updateScreenshot () {
    if (this.status === DisplayStatus.Offline) { this.screenshot = 'assets/img/offline.jpg' } else if (this.status === DisplayStatus.Online) {
    ...
    } else { this.screenshot = 'assets/img/unknown.jpg' }
  }
  ...
  private onResize () {
    if (this.sizeToScreenHeigt) {
      const ImageFramename: string = 'ScreenshotImage'
      const height: number = this.resizer.GetMaxHeight(document.getElementById(ImageFramename)) - 100
